...
public class EntityUtils {
	...
	public Map<String, Class<? extends AMongoObject>> getAMongoObjectMap() {
		...
		Map<String, Class<? extends AMongoObject>> res = new ConcurrentHashMap<>();
	...
	static public void entitySetField(AEntity ae, String field, Object value)
	...
	static public Object entityGetField(AEntity ae, String field)
			throws IllegalArgumentException, IllegalAccessException {
		Field f = ReflectionUtils.findField(ae.getClass(), field);
		ReflectionUtils.makeAccessible(f);
		return f.get(ae);
	...
	static public List<String> readFile(String filename) {
		...
		log.info("Loading list {} ...", filename);

		try (InputStream is = new FileInputStream(filename)) {
			res = new BufferedReader(new InputStreamReader(is,
					StandardCharsets.UTF_8)).lines().collect(Collectors.toList());

			log.info("Finished loading list {}", filename);
		} catch (Exception e) {
